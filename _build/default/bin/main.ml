...
let rec take_public_key (acc:args) : args =
    ...
    |> Sectool.Elliptic_curve.string_of_point
    in
    take_file acc f "Please enter a private key file:"

let rec take_field (acc:args) : args =
    let f s =
    Sectool.File_wizard.read_domain_params s
    |> Sectool.Elliptic_curve.string_of_field
    in
    take_file acc f "Please enter a field file:"
      ...
      match Sectool.Command.parse phrase with
        | Read t ->
            ...
            (match Sectool.File_wizard.read_file file with
                | s -> (
                    ...
                    | 9 -> print_endline (
                        "\n" ^
                        (Sectool.File_wizard.read_domain_params file
                        |> Sectool.Elliptic_curve.string_of_field))
                    | _ ->
        ...
        | Public ->
            let list =
                ...
                |> take_string "Please enter the name of the file to write to:"
            in
            let field = List.hd list |> Sectool.File_wizard.read_domain_params  in
            let private_key = List.nth list 1 |> Sectool.File_wizard.read_private_key in
            let name = List.nth list 2 in
            let result = Sectool.Ecdh.compute_public_key field private_key in
            let result_string = Sectool.Elliptic_curve.string_of_point result in
        ...
        | Secret ->
            ...
            let field = List.hd list |> Sectool.File_wizard.read_domain_params  in
            let private_key = List.nth list 1 |> Sectool.File_wizard.read_private_key in
            let public_key = List.nth list 2 |> Sectool.File_wizard.read_public_key in
            let name = List.nth list 3 in
            let result = Sectool.Ecdh.compute_shared_secret field private_key public_key in
            let result_string = Sectool.Elliptic_curve.string_of_point result in
            Sectool.File_wizard.write_public_key result name;
            print_endline (
